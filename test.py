...
def euler_from_quaternion(rot):
        ...
        return np.array([math.degrees(roll_x), math.degrees(pitch_y), math.degrees(yaw_z)]) # in radians
...
def recover_centre_w_h(x, size):
    img_w, img_h = size
    final = x * torch.tensor([img_w, img_h,
                          img_w, img_h
                          ], dtype=torch.float32)
    return final
...
def infer(images_path, model, i2d, postprocessors, device, output_path):
    ...
    for img_sample in images_path:
        ...
        coupled_bbox = recover_centre_w_h(outputs['pred_boxes'], orig_image.size) # Gives you cx, cy, w, h
        ...
        scaled_centers = rescale_center([center_x, center_y],orig_image.size, [depth_img.shape[1], depth_img.shape[0] ])
        ...
        print(tz)
        new_pose = compute_new_poses(tz, 607.499, 607.42, 325, 231.3, center_x, center_y)
        print(new_pose)
...
if __name__ == "__main__":
    ...
    i2d.load_state_dict(torch.load('depth_net/fyn_model.pt'.format(LOAD_DIR),map_location='cpu'))
    print("model loaded")
